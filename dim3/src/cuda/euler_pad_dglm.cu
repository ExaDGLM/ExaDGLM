...
__global__ void calc_fluxLM(
    ...
    pd_fluxLM[0][pidx] = (n_dot_flux1 - tau[fidx]*(ub1 - pd_u[0][idxM]))*fs;
    pd_fluxLM[1][pidx] = (n_dot_flux2 - tau[fidx]*(ub2 - pd_u[1][idxM]))*fs;
    pd_fluxLM[2][pidx] = (n_dot_flux3 - tau[fidx]*(ub3 - pd_u[2][idxM]))*fs;
    pd_fluxLM[3][pidx] = (n_dot_flux4 - tau[fidx]*(ub4 - pd_u[3][idxM]))*fs;
    pd_fluxLM[4][pidx] = (n_dot_flux5 - tau[fidx]*(ub5 - pd_u[4][idxM]))*fs;
}
...
__global__ void calc_fluxes(
    ...
    pd_fluxR[0][pidx] = rx[ei]*fluxF1 + ry[ei]*fluxG1 + rz[ei]*fluxW1;
    pd_fluxR[1][pidx] = rx[ei]*fluxF2 + ry[ei]*fluxG2 + rz[ei]*fluxW2;
    pd_fluxR[2][pidx] = rx[ei]*fluxF3 + ry[ei]*fluxG3 + rz[ei]*fluxW3;
    pd_fluxR[3][pidx] = rx[ei]*fluxF4 + ry[ei]*fluxG4 + rz[ei]*fluxW4;
    pd_fluxR[4][pidx] = rx[ei]*fluxF5 + ry[ei]*fluxG5 + rz[ei]*fluxW5;
    pd_fluxS[0][pidx] = sx[ei]*fluxF1 + sy[ei]*fluxG1 + sz[ei]*fluxW1;
    pd_fluxS[1][pidx] = sx[ei]*fluxF2 + sy[ei]*fluxG2 + sz[ei]*fluxW2;
    pd_fluxS[2][pidx] = sx[ei]*fluxF3 + sy[ei]*fluxG3 + sz[ei]*fluxW3;
    pd_fluxS[3][pidx] = sx[ei]*fluxF4 + sy[ei]*fluxG4 + sz[ei]*fluxW4;
    pd_fluxS[4][pidx] = sx[ei]*fluxF5 + sy[ei]*fluxG5 + sz[ei]*fluxW5;
    pd_fluxT[0][pidx] = tx[ei]*fluxF1 + ty[ei]*fluxG1 + tz[ei]*fluxW1;
    pd_fluxT[1][pidx] = tx[ei]*fluxF2 + ty[ei]*fluxG2 + tz[ei]*fluxW2;
    pd_fluxT[2][pidx] = tx[ei]*fluxF3 + ty[ei]*fluxG3 + tz[ei]*fluxW3;
    pd_fluxT[3][pidx] = tx[ei]*fluxF4 + ty[ei]*fluxG4 + tz[ei]*fluxW4;
    pd_fluxT[4][pidx] = tx[ei]*fluxF5 + ty[ei]*fluxG5 + tz[ei]*fluxW5;
}
...
void update_rhs(
    ...
    cublasTGEMMBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                NP, nelem, NP,
                &one, pd_WDr, pNP, pd_fluxR, pNP, &zero, pd_utmp, pNP, NVAR);
    
    cublasTGEMMBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                NP, nelem, NP,
                &one, pd_WDs, pNP, pd_fluxS, pNP, &one, pd_utmp, pNP, NVAR);
    
    cublasTGEMMBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                NP, nelem, NP,
                &one, pd_WDt, pNP, pd_fluxT, pNP, &one, pd_utmp, pNP, NVAR);
    
    cublasTGEMMBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                NP, nelem, NFACE*NFP,
                &minus, pd_LIFT, pNP, pd_fluxLM, pNFF, &one, pd_utmp, pNP, NVAR);
