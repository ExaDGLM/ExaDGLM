...
__global__ void init_u(
    ...
    // setup parameters - original setup parameters
    //REAL vx =  1.;
    //REAL vy = -0.5;
    //REAL vz =  1.;
    //REAL pres = 1.;
    ...
    // u4: z-momentum
    // u5: energy
    //REAL u1 = 1 + 0.2*sin(2*PI*(x + y + z));  //# original u1
    
    //# setup parameters - test setup parameters
    REAL vx =  1.;
    REAL vy =  0.;
    REAL vz =  0.;
    REAL pres = 1.;
    ...
    //# test value - u1
    REAL u1 = x*(1-x)*(y+z) + y*(1-y)*(x+z) + z*(1-z)*(x+y);
