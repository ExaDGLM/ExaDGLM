...
__global__ void calc_fluxes(
    ...
    fluxR[idx] = rx[ei]*fluxF + ry[ei]*fluxG + rz[ei]*fluxW;
    fluxS[idx] = sx[ei]*fluxF + sy[ei]*fluxG + sz[ei]*fluxW;
    fluxT[idx] = tx[ei]*fluxF + ty[ei]*fluxG + tz[ei]*fluxW;
}
...
void update_rhs(
    ...
    cublasTGEMM(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                NP, nelem, NP,
                &one, WDr, NP, fluxR, NP, &zero, utmp, NP);
    
    cublasTGEMM(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                NP, nelem, NP,
                &one, WDs, NP, fluxS, NP, &one, utmp, NP);
    
    cublasTGEMM(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                NP, nelem, NP,
                &one, WDt, NP, fluxT, NP, &one, utmp, NP);
    
    cublasTGEMM(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                NP, nelem, NFACE*NFP,
                &minus, LIFT, NP, fluxLM, NFACE*NFP, &one, utmp, NP);
