...
public:
    ...
    void alloc_dev_arrays() {
        ...
        cudaMalloc(&u,      (nelem*NP + buf_size*NFP)*sizeof(REAL));
        cudaMalloc(&k,      nelem*NP*sizeof(REAL));
        cudaMalloc(&utmp,   nelem*NP*sizeof(REAL));
        cudaMalloc(&fluxR,  nelem*NP*sizeof(REAL));
        cudaMalloc(&fluxS,  nelem*NP*sizeof(REAL));
        cudaMalloc(&fluxT,  nelem*NP*sizeof(REAL));
        cudaMalloc(&fluxLM, nelem*NFACE*NFP*sizeof(REAL));
