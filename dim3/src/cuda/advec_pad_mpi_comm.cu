...
__global__ void copy_send_buffer_nfp(
    ...
    if (idx >= data_count) return;
    
    int sidx = sendbuf_idxs[start_idx+idx];
    sendbuf[start_idx + idx] = u[sidx];
}
...
__global__ void copy_recv_buffer_nfp(
    ...
    u[nelem*NP + start_idx + idx] = recvbuf[start_idx + idx];
}
//-----------------------------------------------------------------------------
...
public:
    ...
    void sendrecv_u(DataHost &host, DataDev &dev) {
        int bpg;
        ...
        for (int j=0; j<host.comm_size; j++) {
            ...
            bpg = data_count/TPB + 1;
            copy_send_buffer_nfp<<<bpg,TPB>>>(
                start_idx, data_count, dev.sendbuf_nfp_idxs, dev.u, dev.sendbuf_nfp);
        }
        ...
        for (int j=0; j<host.comm_size; j++) {
            ...
            copy_recv_buffer_nfp<<<bpg,TPB>>>(
                host.nelem, start_idx, data_count, dev.recvbuf_nfp, dev.u);
        }
    }
};
