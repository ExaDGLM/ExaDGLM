...
public:
    ...
    vector<REAL> u, k, utmp, ub;
    vector<REAL> fluxR, fluxS, fluxT, fluxLM;
    ...
    DataHost(int myrank) : myrank(myrank) {
        read_dim();
        alloc_vec_arrays();
        init_vec_arrays();
    }
    ...
    void alloc_vec_arrays() {        
        ...
        // governing variable arrays
        u.resize(nelem*NP + buf_size*NFP);    // MPI communication
        k.resize(nelem*NP, 0);                // zero initialization
        utmp.resize(nelem*NP);
        ub.resize(nelem*NFACE*NFP);
        fluxR.resize(nelem*NP);
        fluxS.resize(nelem*NP);
        fluxT.resize(nelem*NP);
        fluxLM.resize(nelem*NFACE*NFP);
    ...
    void write_u(int tstep) {
        ...
        write_bin_file<REAL>(u0, dataout + fname);
    }
};
