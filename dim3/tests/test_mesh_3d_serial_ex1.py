...
def mesh_elem_ex1(op_face_N3):
    gmsh_fpath = path.join(path.cwd, "gmsh", "ex1.tet.pbc.dx0.4.msh")
    mesh_elem = Gmsh3DSingle(gmsh_fpath)    
    return mesh_elem


@pytest.fixture(scope="module")
def mesh_py(op_face_N3, mesh_elem_ex1):    
    return Mesh3D(op_face_N3, mesh_elem_ex1, mod_suffix='py')


@pytest.fixture(scope="module")
def mesh_cpp(op_face_N3, mesh_elem_ex1):
    return Mesh3D(op_face_N3, mesh_elem_ex1)  # mod_suffix='cpp', default
