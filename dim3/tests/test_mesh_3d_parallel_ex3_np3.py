...
@pytest.mark.parametrize("npart", [2,3,4,5,6,7,8,9,10])
@pytest.fixture(scope="module")
def mesh_elem_ex3_np3(op_face_N3):
    gmsh_fpath = path.join(path.cwd, "gmsh", "ex3.tet.metis.dx0.4.np3.msh")
        
    mesh_elem1 = Gmsh3DPartition(gmsh_fpath, mypart=1)
    mesh_elem2 = Gmsh3DPartition(gmsh_fpath, mypart=2)
    mesh_elem3 = Gmsh3DPartition(gmsh_fpath, mypart=3)
    
    return [mesh_elem1, mesh_elem2, mesh_elem3]


@pytest.fixture(scope="module")
def mesh_py_list(op_face_N3, mesh_elem_ex3_np3):
    return [Mesh3D(op_face_N3, m_e, mod_suffix='py') for m_e in mesh_elem_ex3_np3]


@pytest.fixture(scope="module")
def mesh_cpp_list(op_face_N3, mesh_elem_ex3_np3):
    return [Mesh3D(op_face_N3, m_e) for m_e in mesh_elem_ex3_np3]  # mod_suffix='cpp', default
