...
def mesh_elem_uniform_pbc(op_face_N3):
    x1, x2, nx = 0, 1.4, 8
    y1, y2, ny = 0, 1.2, 7
    z1, z2, nz = 0, 1.0, 6
    mesh_elem = Uniform3DSingle(nx, ny, nz, x1, x2, y1, y2, z1, z2)    
    return mesh_elem


@pytest.fixture(scope="module")
def mesh_py(op_face_N3, mesh_elem_uniform_pbc):    
    return Mesh3D(op_face_N3, mesh_elem_uniform_pbc, mod_suffix='py')


@pytest.fixture(scope="module")
def mesh_cpp(op_face_N3, mesh_elem_uniform_pbc):
    return Mesh3D(op_face_N3, mesh_elem_uniform_pbc)  # mod_suffix='cpp', default
