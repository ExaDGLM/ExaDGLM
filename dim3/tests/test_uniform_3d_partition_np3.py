...
msh3 = Uniform3DPartition(3, npart, nx, ny, nz, x1, x2, y1, y2, z1, z2, bc)
...
def test_VXYZ():
    ...
    assert_ae(msh3.VXYZ[  : 4], [[2,1,2], [2,1,3], [2,1,4], [2,1,5]])
    assert_ae(msh3.VXYZ[ 8:12], [[2,3,2], [2,3,3], [2,3,4], [2,3,5]])
    assert_ae(msh3.VXYZ[12:16], [[3,1,2], [3,1,3], [3,1,4], [3,1,5]])
    assert_ae(msh3.VXYZ[20:  ], [[3,3,2], [3,3,3], [3,3,4], [3,3,5]])
...
def test_consistency_buffer():
    ...
    assert_ae(len(msh1.part2faces[2]), buf_size//2)
    assert_ae(len(msh1.part2faces[3]), buf_size//2)
    assert_ae(len(msh2.part2faces[1]), buf_size//2)
    assert_ae(len(msh2.part2faces[3]), buf_size//2)
    assert_ae(len(msh3.part2faces[1]), buf_size//2)
    assert_ae(len(msh3.part2faces[2]), buf_size//2)
    
    assert_ae(len(msh1.nbrFace2bufIdxs), buf_size)
    assert_ae(len(msh2.nbrFace2bufIdxs), buf_size)
    assert_ae(len(msh3.nbrFace2bufIdxs), buf_size)
    ...
    idx = 0    
    for nbr_part in [2, 3]:
        for eidx, fidx, ridx in msh1.part2faces[nbr_part]:
            sendbuf1[idx][:] = (1, msh1.elemIdx2Tags[eidx], fidx)
            idx += 1
    ...
    idx = 0
    for nbr_part in [1, 3]:
        for eidx, fidx, ridx in msh2.part2faces[nbr_part]:
            sendbuf2[idx][:] = (2, msh2.elemIdx2Tags[eidx], fidx)
            idx += 1
    ...
    #
    # buffers in partition 3
    #
    sendbuf3 = np.zeros((buf_size,3), 'i4')  # (partTag, elemIdx, fidx)
    recvbuf3 = np.zeros((buf_size,3), 'i4')  # (partTag, nbr_elemTag, nbr_fidx)
        
    idx = 0
    for nbr_part in [1, 2]:
        for eidx, fidx, ridx in msh3.part2faces[nbr_part]:
            sendbuf3[idx][:] = (3, msh3.elemIdx2Tags[eidx], fidx)
            idx += 1
    
    for (partTag, elemTag, fidx), buf_idx in msh3.nbrFace2bufIdxs.items():
        recvbuf3[buf_idx][:] = (partTag, elemTag, fidx) 
        
    #
    # check buffer consistency
    #
    half = buf_size//2
    assert_ae(sendbuf1[:half], recvbuf2[:half])
    assert_ae(sendbuf1[half:], recvbuf3[:half])
    assert_ae(sendbuf2[:half], recvbuf1[:half])
    assert_ae(sendbuf2[half:], recvbuf3[half:])
    assert_ae(sendbuf3[:half], recvbuf1[half:])
    assert_ae(sendbuf3[half:], recvbuf2[half:])
