...
@pytest.fixture(scope="module", params=[2,3,4,5,6,7,8,9,10])
def mesh_elem_ex4_np(op_face_N3, request):
    npart = request.param
    gmsh_fpath = path.join(path.cwd, "gmsh", f"ex4.tet.metis.dx0.1.np{npart}.msh")
    mesh_elem_list = [Gmsh3DPartition(gmsh_fpath, mypart) for mypart in range(1,npart+1)]
    
    return mesh_elem_list


@pytest.fixture(scope="module")
def mesh_py_list(op_face_N3, mesh_elem_ex4_np):
    return [Mesh3D(op_face_N3, m_e, mod_suffix='py') for m_e in mesh_elem_ex4_np]


@pytest.fixture(scope="module")
def mesh_cpp_list(op_face_N3, mesh_elem_ex4_np):
    return [Mesh3D(op_face_N3, m_e) for m_e in mesh_elem_ex4_np]  # mod_suffix='cpp', default
